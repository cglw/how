...
public class StatusBarUtil {
    ...
    public static void setTranslucentForImageView(Activity activity, int statusBarAlpha, View needOffsetView) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            LogUtil.i(TAG, "不符合SDK版本----return");
            return;
        }
        LogUtil.i(TAG, "符合SDK版本----开始设置");
        setTransparentForWindow(activity);
        addTranslucentView(activity, statusBarAlpha);
    ...
    public static void setTranslucentForImageViewInFragment(Activity activity, int statusBarAlpha, View needOffsetView) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            LogUtil.i(TAG, "清除Activity的Setting------->开始");
    ...
    private static void clearPreviousSetting(Activity activity) {
        ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
        View fakeStatusBarView = decorView.findViewById(FAKE_STATUS_BAR_VIEW_ID);
        LogUtil.i(TAG, "Setting寻找fakeStatusBarView");
        if (fakeStatusBarView != null) {
            LogUtil.i(TAG, "fakeStatusBarView不为空则移除");
    ...
    private static void addTranslucentView(Activity activity, int statusBarAlpha) {
        LogUtil.i(TAG, "添加半透明矩形条------->start");
        ViewGroup contentView = (ViewGroup) activity.findViewById(android.R.id.content);
        View fakeTranslucentView = contentView.findViewById(FAKE_TRANSLUCENT_VIEW_ID);
        if (fakeTranslucentView != null) {
            LogUtil.i(TAG, "fakeTranslucentView不为null");
            if (fakeTranslucentView.getVisibility() == View.GONE) {
                fakeTranslucentView.setVisibility(View.VISIBLE);
            }
            fakeTranslucentView.setBackgroundColor(Color.argb(statusBarAlpha, 0, 0, 0));
        } else {
            LogUtil.i(TAG, "fakeTranslucentView为null");

            contentView.addView(createTranslucentStatusBarView(activity, statusBarAlpha));
            LogUtil.i(TAG, "add  fakeTranslucentView");
        }
    }
    ...
    private static void setTransparentForWindow(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            LogUtil.i(TAG, ">21设置Activity状态栏透明");
            activity.getWindow().setStatusBarColor(Color.TRANSPARENT);
            activity.getWindow()
                    .getDecorView()
                    .setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            LogUtil.i(TAG, ">19设置Activity状态栏透明");
            activity.getWindow()
                    .setFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS, WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        }
        LogUtil.i(TAG, "设置Activity状态栏透明--->complete");
    }
