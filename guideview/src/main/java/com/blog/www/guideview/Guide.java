...
public class Guide implements View.OnKeyListener, View.OnClickListener {
    ...
    public void show(Activity activity) {
        ...
        if (fullViews != null) {
            for (int i = 0; i < fullViews.size(); i++) {
                if (fullViews.get(i).getParent() == null) {
                    contentView.addView(fullViews.get(i));
                }
            }
        }
    }
    ...
    private MaskView onCreateView(Activity activity) {
        ...
        if (mConfiguration.mTargetView != null) {
            target = mConfiguration.mTargetView;
        ...
        if (target != null) {
            getViewTreeObserver = new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    Log.e("getViewTreeObserver", loc[0] + "," + loc[1]);
                    if (lastloc[0] != loc[0] || lastloc[1] != loc[1]) {
                        move();
                    }
                    lastloc[0] = loc[0];
                    lastloc[1] = loc[1];
            ...
            target.getViewTreeObserver().addOnGlobalLayoutListener(getViewTreeObserver);
        }
        ...
        for (Component c : mComponents) {
            if (c.LayoutFullScreen()) {
                if (contentView != null) {
                    if (fullViews == null)
                        fullViews = new ArrayList<>();
                    fullViews.add(Common.FullToView(activity.getLayoutInflater(), c));
                }
            } else
    ...
    public void move() {
        if (contentView == null || mMaskView == null)
            return;
        int parentX = 0;
        int parentY = 0;
        final int[] loc = new int[2];
        ...
        if (mConfiguration.mTargetView != null) {
            mMaskView.setTargetRect(Common.getViewAbsRect(mConfiguration.mTargetView, parentX, parentY));
        } else {
            // Gets the target view's abs rect
            View target = activity.findViewById(mConfiguration.mTargetViewId);
            if (target != null) {
                mMaskView.setTargetRect(Common.getViewAbsRect(target, parentX, parentY));
            }
        }
        View fulling = activity.findViewById(mConfiguration.mFullingViewId);
        if (fulling != null) {
            mMaskView.setFullingRect(Common.getViewAbsRect(fulling, parentX, parentY));
        }
    ...
    private void onDestroy() {
        ...
        mMaskView = null;
        if (target != null && getViewTreeObserver != null)
            target.getViewTreeObserver().removeOnGlobalLayoutListener(getViewTreeObserver);
        target = null;
        if (contentView != null) {
            if (fullViews != null) {
                for (int i = 0; i < fullViews.size(); i++) {
                    contentView.removeView(fullViews.get(i));
                }
            }
        }
    }
